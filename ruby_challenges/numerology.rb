...
if number > 9 #ability to loop?
  number = number.to_s
  number = number[0].to_i + number[1].to_i
  puts "Your number is #{number}!"
else puts "Your number is #{number}!"
end #sk code has this puts in case stmt w/ \n

#number = number.to_s - not needed,
case number #keep as an integer = shorter code (?)
when 1
